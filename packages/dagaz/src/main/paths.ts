...
// before any service is constructed. Under the sidecar (once it lands for
// Dagaz — currently deferred pending the OAuth flow rewrite) it supplies
// ~/.futhark/dagaz.
...
export function userDataDir(): string {
  if (!_paths) {
    throw new Error(
      '[Dagaz] paths not configured — call configurePaths() before constructing services',
    );
  }
