...
/**
 * Service resolver: returns the current active account's services.
 * When a resolver is provided, it takes precedence over the static instances.
 * This allows the API server to follow account switches.
 */
...
export function startApiServer(gmail: GmailService, hubspot: HubSpotService, port: number, viewStore?: ViewStore, ruleStore?: RuleStore, calendar?: CalendarService, configStore?: ConfigStore, getMainWindow?: () => import('electron').BrowserWindow | null, cacheStore?: CacheStore, resolver?: ServiceResolver) {
  const app = express();
  app.use(express.json({ limit: '50mb' }));

  const g = () => resolver?.gmail() ?? gmail;
  const hs = () => resolver?.hubspot() ?? hubspot;
  const cal = () => resolver?.calendar() ?? calendar;
  const cfg = () => resolver?.configStore() ?? configStore;
  ...
  app.get('/api/inbox', async (_req, res) => {
    try {
      const result = await g().fetchThreads('in:inbox', 50);
  ...
  app.get('/api/unread', async (_req, res) => {
    try {
      const result = await g().fetchThreads('is:unread in:inbox', 50);
  ...
  app.get('/api/email/:id', async (req, res) => {
    try {
      const thread = await g().fetchThread(req.params.id);
  ...
  app.get('/api/thread/:id/summary', async (req, res) => {
    try {
      const summary = await g().getThreadSummary(req.params.id);
      res.json(summary);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/search', async (req, res) => {
    try {
      ...
      const result = await g().fetchThreads(q, 50);
      res.json({ threads: result.threads });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post('/api/send', async (req, res) => {
    try {
      const result = await g().sendEmail(req.body);
      res.json(result);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post('/api/draft', async (req, res) => {
    try {
      const draftId = await g().createDraft(req.body);
      res.json({ draftId });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/drafts', async (_req, res) => {
    try {
      const drafts = await g().listDrafts();
      res.json({ drafts });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/labels', async (_req, res) => {
    try {
      const labels = await g().listLabels();
      res.json({ labels });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/attachment/:messageId/:attachmentId', async (req, res) => {
    try {
      ...
      const buffer = await g().getAttachmentBuffer(req.params.messageId, req.params.attachmentId);
  ...
  app.get('/api/thread/:threadId/attachments', async (req, res) => {
    try {
      const attachments = await g().listThreadAttachments(req.params.threadId);
  ...
  app.post('/api/attachment/:messageId/:attachmentId/download', async (req, res) => {
    try {
      const filename = (req.query.filename as string) || 'attachment';
      const filePath = await g().downloadAttachment(
  ...
  app.get('/api/thread/:threadId/attachments/download-all', async (req, res) => {
    try {
      ...
      for (const att of attachments) {
        const buffer = await g().getAttachmentBuffer(att.messageId, att.id);
  ...
  app.get('/api/config', (_req, res) => {
    ...
    const cfgData = configStore.get();
    res.json({
      archiveOnReply: cfgData.archiveOnReply,
    });
  });
  ...
  app.get('/api/hubspot/contact/:email', async (req, res) => {
    try {
      const context = await hs().lookupContact(req.params.email);
      res.json(context);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/hubspot/recent/:email', async (req, res) => {
    try {
      ...
      const result = await hs().getRecentActivities(req.params.email, limit);
      res.json(result);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.post('/api/hubspot/log', async (req, res) => {
    try {
      await hs().logEmail(req.body, req.body.gmail_message_id || '');
      res.json({ success: true });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  app.get('/api/context/:email', async (req, res) => {
    try {
      ...
      const [hubspotContext, emailThreads] = await Promise.all([
        hs().lookupContact(email).catch(() => ({ contact: null, deals: [], activities: [], loading: false, error: null })),
        g().fetchThreads(`from:${email} OR to:${email}`, 5).catch(() => ({ threads: [], nextPageToken: undefined })),
      ]);
  ...
  app.get('/api/views', (_req, res) => {
    const store = vs();
    if (!store) return res.status(503).json({ error: 'Views not available' });
    res.json({ views: store.list() });
  });
  ...
  app.put('/api/views/:id', (req, res) => {
    const store = vs();
    if (!store) return res.status(503).json({ error: 'Views not available' });
    const views = store.update(req.params.id, req.body);
    res.json({ views });
  });
  ...
  app.delete('/api/views/:id', (req, res) => {
    const store = vs();
    if (!store) return res.status(503).json({ error: 'Views not available' });
    const views = store.remove(req.params.id);
    res.json({ views });
  });
  ...
  app.get('/api/rules', (_req, res) => {
    const store = rs();
    if (!store) return res.status(503).json({ error: 'Rules not available' });
  ...
  app.put('/api/rules/:id', (req, res) => {
    const store = rs();
    if (!store) return res.status(503).json({ error: 'Rules not available' });
    const rules = store.update(req.params.id, req.body);
    res.json({ rules });
  });
  ...
  app.delete('/api/rules/:id', (req, res) => {
    const store = rs();
    if (!store) return res.status(503).json({ error: 'Rules not available' });
    const rules = store.remove(req.params.id);
    res.json({ rules });
  });
  ...
  app.post('/api/calendar/rsvp/:eventId', async (req, res) => {
    const calendar = cal();
    if (!calendar) return res.status(503).json({ error: 'Calendar not available' });
    try {
