...
export function RichTextEditor({ content, onChange, placeholder, autoFocus, onSubmit, minHeight = '80px' }: Props) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      attributes: {
        class: `max-w-none focus:outline-none px-3 py-2 text-sm text-text-primary`,
  ...
  return (
    <div className="flex flex-col border border-border-subtle rounded-lg overflow-hidden bg-bg-primary">
      {/* Compact toolbar */}
      <div className="flex items-center gap-0.5 px-2 py-1 border-b border-border-subtle bg-bg-secondary/50 flex-wrap relative">
        <ToolbarGroup>
        ...
        <ToolbarGroup>
          <ToolbarButton active={editor.isActive('link')} onClick={openLinkPopover} title="Insert link">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {linkPopover.open && (
          <div className="absolute top-full left-0 mt-1 z-50 flex items-center gap-1.5 bg-bg-tertiary border border-border-subtle rounded-lg shadow-lg px-3 py-2 ml-2">
            <input
              ref={linkInputRef}
              type="text"
              ...
              className="w-56 bg-bg-primary border border-border-subtle rounded px-2 py-1 text-xs text-text-primary outline-none focus:border-accent-primary"
              placeholder="https://example.com"
            />
            <button type="button" onClick={applyLink} className="px-2 py-1 bg-accent-primary text-white text-[10px] rounded font-medium hover:bg-accent-primary/80 transition-colors">
              Apply
            </button>
            {editor.isActive('link') && (
              <button type="button" onClick={removeLink} className="px-2 py-1 bg-bg-hover text-text-muted text-[10px] rounded font-medium hover:text-accent-danger transition-colors">
                Remove
              </button>
            )}
            <button
      ...
      {/* Editor area */}
      <div className="overflow-y-auto bg-bg-primary selectable">
        <EditorContent editor={editor} className="prose-raido" />
      </div>
    </div>
  );
}

export function useRichEditorReset() {
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center w-6 h-6 rounded text-[11px] font-medium transition-colors
        ${active ? 'bg-accent-primary/20 text-accent-primary' : 'text-text-muted hover:bg-bg-hover hover:text-text-secondary'}`}
    >
      {children}
    </button>
  );
