          ...
          filteredTasks.map((task, i) => (
            <React.Fragment key={task.id}>
              ...
              >
                ...
                {/* Defer indicator */}
                {task.defer_until && task.defer_until > new Date().toISOString().split('T')[0] && (
                  <span className="flex-shrink-0" title={`Hidden until ${task.defer_until}`} style={{ color: 'var(--color-amber, #f59e0b)' }}>
